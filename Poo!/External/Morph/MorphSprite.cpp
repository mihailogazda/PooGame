...
{
	_timeUniformLocation=0;
    _totalTime=0;
    
    _touch1PositionXUniformLocation =0;
	_touch1PositionYUniformLocation =0;

    _touch1Position = ccp(-1.f,-1.f);
    _touch2Position = ccp(-1.f,-1.f);
...
void MorphSprite::initShader(const char *fshFileName){
	...
	//const GLchar *  fragmentSource =(GLchar *)CCString::createWithContentsOfFile(CCFileUtils::sharedFileUtils()->fullPathForFilename(fshFileName).c_str())->getCString();
